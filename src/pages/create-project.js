...
class EditProject extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.match.params.id !== 'create') {
      ...
      const requestURL = `http://techconnect-api.ddns.net:1337/profiles/${params.id}`;
  ...
  handleSubmit = (e) => {
    ...
    const requestURL =  params.id === 'create' ? `http://techconnect-api.ddns.net:1337/${params.contentType}`: `http://techconnect-api.ddns.net:1337/${params.contentType}/${params.id}`;
        ...
        if (!isEmpty(FILE_RELATIONS[params.contentType])) {
          map(FILE_RELATIONS[params.contentType], (value, key) => {
            if (!isEmpty(modifiedData[value.name])) {
              ...
              return request('http://techconnect-api.ddns.net/upload', { method: 'POST', body, headers: {} }, false)
                .catch(err => {
